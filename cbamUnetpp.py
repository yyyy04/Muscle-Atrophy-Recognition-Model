...
from cbamUnet import CBAM
import torch.nn.functional as F

class cbamUnetPP(SegmentationModel):
    ...
    def __init__(
        ...
        for out_channel in [1, 64, 64, 128, 256, 512]:
            self.cbam_attention.append(CBAM(out_channel))
        ...
        self.name = "CBAM-unetplusplus-{}".format(encoder_name)
        self.initialize()
    ...
    def forward(self, x):
        ...
        feat_maps_cbam = []
        new_features = []
        for f, cbam_attention in zip(features, self.cbam_attention):
            feat_maps_cbam.append((cbam_attention(f)))
        for idx in range(len(features)):
            new_features.append(feat_maps_cbam[idx] + features[idx])
